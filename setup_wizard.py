...
def step_momentum(existing, budget):
    ...
    print(f"  {bold('[5b] 損切り幅プロファイル（ボラティリティ別・自動）')}")
    print(f"  {dim('銘柄のボラに応じて損切り幅を自動調整します（上の基準値に銘柄別の倍率を掛けます）。')}")
    print(f"  {dim('発注サイズは縮小しません ─ ボラの高い銘柄ほど手数料に勝ちやすいため、')}")
    print(f"  {dim('サイズはそのまま・損失の総量は日次/週次の自動停止（サーキットブレーカー）で管理します。')}")
    print()
    print(f"  {dim('  銘柄    区分      狭め    標準    広め')}")
    print(f"  {dim('  SMH     実発注    1.00%   1.50%   2.00%')}")
    print(f"  {dim('  QQQ     実発注    0.70%   1.00%   1.30%')}")
    print(f"  {dim('  SPY     実発注    0.50%   0.65%   0.85%')}")
    print(f"  {dim('  DRAM    シャドー  0.70%   1.00%   1.30%')}")
    print(f"  {dim('  IWM     シャドー  0.35%   0.50%   0.65%')}")
    ...
    ok_box([
        ...
        ("損切り幅プロファイル",     f"{_prof_name}  (ボラ別・自動)"),
...
def _build_env_lines(config):
    ...
    return [
        ...
        "#   MOMENTUM_STOP_PROFILE      損切り幅プロファイル (ボラ別・Wizard [5b] で選択)",
